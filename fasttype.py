...
def userInput(event):
        ...
        global color
        global Letes
        print("Word length : ",len(rndw))
        
        if(gameOv == False):
                if(event.char in rndw.lower() and (len(userW) != len(rndw))):
                        userW += event.char
                        print(event.char)
                        point += 1
                        canvas.delete(ALL)
                        ...
                        point += 0
                elif(event.char not in rndw.lower()):
                        userW += event.char
                        print("You typed : ",userW)
                        point -= 1
                        canvas.delete(ALL)
                        word(rndw,point,tLeft)
                if(userW.lower() == rndw.lower()):
